...
server.get('/category/:nombreCat', function (req, res, next) {
	Category.findOne({
		...
		.then(function (products) {
			if (!products) { return res.send('no existe esa categoria').status(404); }
			if (products == [] || products == 0) { return res.send('no existen productos para esa categoria').status(204); }

			return res.status(200).send({ data: { count: products.length, rows: products } });
		}).catch(err => {
			console.log('err');
