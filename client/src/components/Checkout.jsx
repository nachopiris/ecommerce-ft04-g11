...
function Checkout({token, user, userCart, createOrder, getCollect})
{
    ...
    const onSubmit = (data) => {
        ...
        createOrder({...data, token}).then((res) => {
                    ...
                    <Card className="bg-dark2">
                        ...
                        <Card.Body>
                            <div className="text-center mt-3">
                                ...
                                <h2 className="mb-4">${state.total}</h2>
                            </div>
                            <div className="w-75 m-auto pb-4">
                                {state.products.map((item, index) => (
                                    <div key={index} className="p-3 shadow d-flex justify-content-between">
                                        <span>
                                            <span className="text-muted">{item.name}</span>
                                            {" "}
                                            <span className="badge badge-dark">{item.quantity} x ${item.price}</span>
                                        </span>
                                        <span>
                                            ${item.subtotal}
                                        </span>
                            ...
                            <div className="w-75 m-auto shadow rounded p-3 my-2">
                                {(!user.address || !user.doc_number || !user.phone) && (
                                    <p className="text-muted py-4">
                                        <b>{user.fullname.split(' ')[0]}</b>, necesitamos algunos datos para completar la compra. <b>Solo tomará un minuto!</b>
                                    </p>
                                )}
                                    <Form.Group>
                                        <Form.Label>N° Documento <span className="text-danger">*</span></Form.Label>
                                        <Form.Control
                                            autoComplete="off"
                        ...
                        <Card.Footer className="text-center p-4">
                            <div className="w-75 m-auto pb-3">
                                <Button type="submit" className="text-uppercase m-auto" disabled={state.loading} block size="lg">
